...
CREATE TABLE notificaciones_protectora(
    ...
    CONSTRAINT fk_not_protectora
        FOREIGN KEY (protectora_id)
        REFERENCES protectora(protectora_id)
        ON DELETE CASCADE,
        
    CONSTRAINT fk_not_cliente
...
CREATE TABLE auditoria_notificaciones (
    ...
    tipo_notificacion VARCHAR2(30), -- 'Cliente' o 'Protectora'
...
-- Disparadores notificaciones cliente
CREATE OR REPLACE TRIGGER trg_auditoria_notif_cliente
BEFORE DELETE ON notificaciones_cliente
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_notificaciones (
        tipo_notificacion, id_notificacion, nueva_cita, informacion,
        protectora_id, cliente_id, fecha_alta, fecha_modificacion
    ) VALUES (
        'Cliente',
        :OLD.id_notificacion, :OLD.nueva_cita, :OLD.informacion,
        :OLD.protectora_id, :OLD.cliente_id,
        :OLD.fecha_alta, :OLD.fecha_modificacion
    );
END;
/

-- Disparadores notificaciones protectora
CREATE OR REPLACE TRIGGER trg_auditoria_notif_protectora
BEFORE DELETE ON notificaciones_protectora
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_notificaciones (
        tipo_notificacion, id_notificacion, nueva_cita, informacion,
        ...
        'Protectora',
        :OLD.id_notificacionP, :OLD.nueva_cita, :OLD.informacion,
        :OLD.protectora_id, :OLD.cliente_id,
        :OLD.fecha_alta, :OLD.fecha_modificacion
    );
END;
